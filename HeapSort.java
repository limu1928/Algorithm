...
public class HeapSort {
  ...
  public void Heapify(List<Integer> list, int i){
    ...
    if(i >=0 && i < len){
      ...
      int largest = i;
      if(leftIndex < len) { if(list.get(leftIndex) > list.get(i)) largest = leftIndex;}
      if(rightIndex < len){if(list.get(rightIndex) > list.get(largest)) largest = rightIndex;}
      if(largest != i){
        swap(list,largest,i);
  ...
  public Integer extractMax(List<Integer> heap){
    int len = heap.size();
    if(len > 0){
